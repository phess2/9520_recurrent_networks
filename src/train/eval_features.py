...
from .train import compute_metrics, shift_targets, shift_targets_ids, maybe_cast_precision
...
def _load_params(model: BaseSequenceModel, key: jax.Array, checkpoint_path: str | None):
	if checkpoint_path:
		if not _ORBAX_AVAILABLE:
			raise RuntimeError("Orbax is required to restore checkpoints but is unavailable.")
...
def _build_task_and_sampler(task_cfg: Dict[str, Any]) -> Tuple[Dict[str, int], Dict[str, Any], Callable[[], Dict[str, jnp.ndarray]]]:
...
def _evaluate_batch(model: BaseSequenceModel, params: Any, batch: Dict[str, jnp.ndarray], task_info: Dict[str, Any], precision: str):
...
def evaluate(model: BaseSequenceModel, params: Any, task_info: Dict[str, Any], sample_batch_fn: Callable[[], Dict[str, jnp.ndarray]], eval_cfg: FeatureEvalConfig):
	...
	for batch_idx in range(eval_cfg.num_batches):
		...
		metrics, obs, mask = _evaluate_batch(model, params, batch, task_info, eval_cfg.precision)
...
def main(cfg: DictConfig) -> None:
	...
	metrics, feature_records = evaluate(model, params, task_info, sample_batch_fn, eval_cfg)
	print("Aggregate metrics:", metrics)
	if feature_records:
		summary_path = os.path.join(eval_cfg.output_dir, "summary.npz")
		means = {
			"frobenius_mean": np.mean([np.mean(rec["frobenius_norms"]) for rec in feature_records]),
			"active_fraction_mean": np.mean([np.mean(rec["nonlinearity_active_fraction"]) for rec in feature_records]),
			"scaling_mean": np.mean([np.mean(rec["nonlinearity_scaling"]) for rec in feature_records]),
		}
		np.savez(summary_path, **means)
		print(f"Saved feature artifacts to {eval_cfg.output_dir}")
	else:
		print("Model does not expose analyze_batch; no feature artifacts were produced.")
