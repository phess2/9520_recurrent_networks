...
def _leaky_relu_factory(negative_slope: float) -> Nonlinearity:
    ...
    return Nonlinearity(
        name=f"leaky_relu_{negative_slope}", fn=fn, jacobian_diag=jacobian
    )
...
def _gelu_jacobian(x: Array) -> Array:
    ...
    return 0.5 * (1.0 + jax.lax.erf(x / jnp.sqrt(2.0))) + x * inv_sqrt_2pi * jnp.exp(
        -0.5 * x**2
    )
...
def _identity_nonlinearity() -> Nonlinearity:
    return Nonlinearity(
        name="identity", fn=lambda x: x, jacobian_diag=lambda x: jnp.ones_like(x)
    )
...
def get_nonlinearity(name: str, **kwargs) -> Nonlinearity:
    ...
    if builder is None:
        raise ValueError(
            f"Unsupported nonlinearity '{name}'. Available: {sorted(_REGISTRY.keys()) + ['leaky_relu']}"
        )
    return builder()
