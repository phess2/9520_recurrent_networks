...
class TransformerAdapter(BaseSequenceModel):
	...
	def __init__(self, config: ModelConfig, num_heads: int = 4, num_layers: int | None = None):
	...
	@staticmethod
	def _mha(x: jnp.ndarray, layer: Dict[str, jnp.ndarray], num_heads: int) -> jnp.ndarray:
		# x: [B, T, D_model]
		B, T, Dm = x.shape
		H = max(1, num_heads)
	...
	def _build_mlp_fallback(self):
		if not _LINEAR_AVAILABLE:
			raise RuntimeError("Modula Linear not available; install from https://github.com/modula-systems/modula.git")
