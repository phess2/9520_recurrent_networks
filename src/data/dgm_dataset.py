...
class DGMDataset:
	...
	def _init_random_params(self) -> None:
		...
		if c.discrete_latent:
			...
			if len(c.lags) == 0:
				...
				self.trans_logits = jax.random.normal(k, (len(c.lags), c.num_categories, c.num_categories)) * 0.1
			...
			if len(c.lags) == 0:
				...
				self.key, k = jax.random.split(self.key)
				w = jax.random.normal(k, (len(c.lags), c.dims, c.dims)) * (c.cov_scale / jnp.sqrt(c.dims))
				self.trans_weights = w
		# Emission mapping (for continuous), and biases
		self.key, k_e = jax.random.split(self.key)
		self.emission = jax.random.normal(k_e, (c.obs_dims, max(1, c.dims))) * (1.0 / jnp.sqrt(max(1, c.dims)))
	...
	def _emit(self, latent) -> Tuple[jnp.ndarray, Optional[int]]:
		...
		if c.discrete_latent:
			...
			noisy = jnp.clip(one_hot + 0.1 * jax.random.normal(k, (c.obs_dims,)), 0.0, 1.0)
			return noisy, obs_id
		else:
			# y_t = E x_t + b + eps
			self.key, k = jax.random.split(self.key)
			cont = self.emission @ latent + self.emission_bias + self.emission_noise * jax.random.normal(k, (self.config.obs_dims,))
			return cont, None
