...
class KBitFlipFlopDataset:
    ...
    """
    def __init__(self, k: int, batch_size: int, seq_length: int, p: float = 0.01, noise_std: float = 0.01):
        self.k = k
    ...
    def make_kbit_flipflop_batch(self):
        ...
        pulse_mask = jax.random.bernoulli(pulse_key, self.p, shape=(self.batch_size, self.seq_length, self.k))
        # Sign for each pulse (+1 or -1)
        signs = jax.random.choice(sign_key, jnp.array([-1.0, 1.0]), shape=(self.batch_size, self.seq_length, self.k))
        # Pulses (set value for pulse positions, 0 elsewhere)
        pulses = signs * pulse_mask

        # Add noise (always, including pulse times)
        noise = self.noise_std * jax.random.normal(noise_key, (self.batch_size, self.seq_length, self.k))
        inputs = pulses + noise
