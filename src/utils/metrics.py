...
def classification_metrics(logits: jnp.ndarray, targets: jnp.ndarray, mask: jnp.ndarray) -> Dict[str, jnp.ndarray]:
...
def regression_metrics(pred: jnp.ndarray, targets: jnp.ndarray, mask: jnp.ndarray) -> Dict[str, jnp.ndarray]:
	mse = jnp.sum(((pred - targets) ** 2) * mask[..., None]) / jnp.sum(mask)
	nll = 0.5 * mse
	return {"nll": nll, "accuracy": 0.0 * nll}


def mutual_information_placeholder(latents: jnp.ndarray, inferred: jnp.ndarray, mask: jnp.ndarray) -> jnp.ndarray:
